...
class CanvasPanel extends JPanel {
    ...
    // ---- Bézier construction state (4 clicks) ----
    private final ArrayList<Point> bezierPoints = new ArrayList<>();
    private Point bezierPreviewPoint = null; // current mouse position during construction

    // ---- Hermite construction state (click-drag-click-drag) ----
    // Steps: 0=waiting P1, 1=dragging R1, 2=waiting P4, 3=dragging R4
    private int hermiteStep = 0;
    private int hermP1x, hermP1y; // P1
    private int hermR1x, hermR1y; // R1 tangent vector
    private int hermP4x, hermP4y; // P4
    private int hermDragStartX, hermDragStartY; // start of current drag
    private Shape hermitePreview = null;
    ...
    private boolean isDrawMode() {
        return currentMode == ToolMode.DRAW_POINT || currentMode == ToolMode.DRAW_LINE
                ...
                || currentMode == ToolMode.DRAW_BEZIER || currentMode == ToolMode.DRAW_HERMITE;
    }
    ...
    public void setMode(ToolMode mode) {
        ...
        previewShape = null;
        // Reset curve construction state
        bezierPoints.clear();
        bezierPreviewPoint = null;
        hermiteStep = 0;
        hermitePreview = null;
        repaint();
    }
    ...
    public CanvasPanel(CanvasListener listener) {
        ...
        MouseAdapter mouse = new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                } else if (currentMode == ToolMode.DRAW_BEZIER && !bezierPoints.isEmpty()) {
                    // Live preview of Bézier curve while placing control points
                    bezierPreviewPoint = new Point(worldX, worldY);
                    int n = bezierPoints.size();
                    if (n == 1) {
                        // 1 point placed: show line from p0 to cursor
                        Point p0 = bezierPoints.get(0);
                        previewShape = new Line2D(p0.x, p0.y, worldX, worldY);
                    } else if (n == 2) {
                        // 2 points placed: show partial Bézier (use cursor as p2 and p3)
                        Point p0 = bezierPoints.get(0), p1 = bezierPoints.get(1);
                        previewShape = new BezierCurve2D(
                                p0.x, p0.y, p1.x, p1.y, worldX, worldY, worldX, worldY);
                    } else if (n == 3) {
                        // 3 points placed: show Bézier with cursor as p3
                        Point p0 = bezierPoints.get(0), p1 = bezierPoints.get(1), p2 = bezierPoints.get(2);
                        previewShape = new BezierCurve2D(
                                p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, worldX, worldY);
                    }
                } else if (currentMode == ToolMode.DRAW_HERMITE && hermiteStep == 2) {
                    // After R1 defined, waiting for P4 click — show P1 marker
                    hermitePreview = null; // just show the point marker in paint
                }
                repaint();
            }
            ...
            public void mouseReleased(MouseEvent e) {
                if (isDrawMode()) {
                    ...
                    if (currentMode == ToolMode.DRAW_HERMITE) {
                        int worldX = e.getX() - offsetX;
                        int worldY = e.getY() - offsetY;
                        if (hermiteStep == 1) {
                            // Finished dragging R1
                            hermR1x = worldX - hermP1x;
                            hermR1y = worldY - hermP1y;
                            hermiteStep = 2; // next: click P4
                            hermitePreview = null;
                        } else if (hermiteStep == 3) {
                            // Finished dragging R4 — create the curve
                            int r4x = worldX - hermP4x;
                            int r4y = worldY - hermP4y;
                            HermiteCurve2D curve = new HermiteCurve2D(
                                    hermP1x, hermP1y, hermR1x, hermR1y,
                                    hermP4x, hermP4y, r4x, r4y);
                            shapes.add(curve);
                            hermiteStep = 0;
                            hermitePreview = null;
                            previewShape = null;
                        }
                    } else if (currentMode != ToolMode.DRAW_POLYGON
                            && currentMode != ToolMode.DRAW_BEZIER
                            && previewShape != null) {
            ...
            private void handleDrawPress(MouseEvent e) {
                ...
                } else if (currentMode == ToolMode.DRAW_BEZIER) {
                    // Bézier: collect 4 clicks
                    bezierPoints.add(new Point(worldX, worldY));
                    if (bezierPoints.size() == 4) {
                        Point p0 = bezierPoints.get(0), p1 = bezierPoints.get(1);
                        Point p2 = bezierPoints.get(2), p3 = bezierPoints.get(3);
                ...
                } else if (currentMode == ToolMode.DRAW_HERMITE) {
                    // Hermite: step 0 = click P1, step 2 = click P4
                    if (hermiteStep == 0) {
                        hermP1x = worldX; hermP1y = worldY;
                        hermDragStartX = worldX; hermDragStartY = worldY;
                        hermiteStep = 1; // next: drag to define R1
                    } else if (hermiteStep == 2) {
                        hermP4x = worldX; hermP4y = worldY;
                        hermDragStartX = worldX; hermDragStartY = worldY;
                        hermiteStep = 3; // next: drag to define R4
                    }
                } else {
                    previewShape = new Point2D(worldX, worldY);
                }
            ...
            private void handleDrawDrag(int x, int y) {
                ...
                switch (currentMode) {
                    ...
                    case DRAW_POINT:
                        ...
                        break;
                    case DRAW_HERMITE:
                        // Live preview while dragging tangent
                        if (hermiteStep == 1) {
                            // Dragging R1: show partial preview with tangent from P1
                            int r1x = worldX - hermP1x, r1y = worldY - hermP1y;
                            // Show a simple tangent line as preview
                            hermitePreview = new Line2D(hermP1x, hermP1y, worldX, worldY);
                        } else if (hermiteStep == 3) {
                            // Dragging R4: show full Hermite preview
                            int r4x = worldX - hermP4x, r4y = worldY - hermP4y;
                            hermitePreview = new HermiteCurve2D(
                                    hermP1x, hermP1y, hermR1x, hermR1y,
                                    hermP4x, hermP4y, r4x, r4y);
                        }
                        break;
...
@Override
    protected void paintComponent(Graphics g) {
        ...
        // Draw Hermite preview during construction
        if (hermitePreview != null) {
            hermitePreview.draw(g2, offsetX, offsetY, true, listener != null ? (PaintApp) listener : null, vLeft, vTop, vRight, vBottom);
        }

        // Draw Bézier control points already placed (during construction)
        if (currentMode == ToolMode.DRAW_BEZIER && !bezierPoints.isEmpty()) {
            g2.setColor(new Color(0, 120, 0));
            for (Point p : bezierPoints) {
                g2.fillOval(p.x + offsetX - 4, p.y + offsetY - 4, 8, 8);
            }
        }

        // Draw Hermite points already placed (during construction)
        if (currentMode == ToolMode.DRAW_HERMITE && hermiteStep >= 1) {
            g2.setColor(new Color(0, 120, 0));
            g2.fillOval(hermP1x + offsetX - 4, hermP1y + offsetY - 4, 8, 8);
            if (hermiteStep >= 2) {
                // Show R1 tangent arrow
                g2.setColor(new Color(0, 100, 200, 180));
                float[] dashH = {4f, 4f};
                g2.setStroke(new BasicStroke(1.5f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10f, dashH, 0f));
                g2.drawLine(hermP1x + offsetX, hermP1y + offsetY,
                        (hermP1x + hermR1x) + offsetX, (hermP1y + hermR1y) + offsetY);
